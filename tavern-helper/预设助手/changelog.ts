...
export const CHANGELOGS: ChangelogEntry[] = [
  {
    ...
    title: '⚖️日月西 更新日志 |',
    intro:
      '<em>先提前祝大家中秋节快乐~~🎑</em><br><em>忙着殴打电波手机了所以改得比较慢，我们仍然没有等来3.9f或者4.0pro！😡</em><br><em>甚至还命苦地被谷歌标记了——新增了一些头部，如果还是道歉的话推荐换一个时间再玩，有时候build的甲会变得非常厚，这个时候和预设的破甲能力无关</em>',
    bodyHtml: `
      <section class="changelog-section">
        ...
        <ul>
          <li>【万物诞生】：写了两版，一个是原版+随机头部名，一个是多语随机版，选一个开就可以了</li>
          <li>【头部破限】：【Gemini头部】写了两版，2号东西更多更强力，二选一开；【自定义混淆】是提供的自己缝合头部，自行在里面填充内容，小说、论文等各类长文本都行，总字数必须在1w+以上；平时不道歉空回不要开，会导致模型降智，出现道歉空回再尝试开启</li>
        </ul>

        <div class="changelog-subtitle" role="heading" aria-level="3">🎵 文风指导</div>
        <ul>
          <li>新增【散场后】：韩剧文风，来自宝宝的许愿</li>
          <li>新增【结尾落点】模块：一共有五种结尾收尾方式，随机结尾是后面四种结尾随机出一个，多选一开，必须搭配文中检测里的【创作检测】使用</li>
        </ul>

        <div class="changelog-subtitle" role="heading" aria-level="3">✒️ 写作指导</div>
        <ul><li>新增【宠物优化】：防char仇视宠物、对宠物吃醋</li></ul>

        <div class="changelog-subtitle" role="heading" aria-level="3">🎐 装饰组件</div>
        <ul>
          <li>优化【日月秘密来信】：新增了一些来信对象</li>
          <li>修复【浮生绘卷状态】：删掉了多余的&lt;details&gt;</li>
          <li>优化【极光剧场提醒】：补上了剧场cot，挂世界书时生成剧场更稳定</li>
        </ul>

        <div class="changelog-subtitle" role="heading" aria-level="3">💡 预设思维</div>
        <ul>
          <li>新增【智能推进】：更智能地推进事件，跳过不重要的、冗余的部分</li>
          <li>新增【有事业心】：加强去user中心化，发展角色个人生活和事业</li>
        </ul>
      </section>
      ...
      <section class="changelog-section">
        ...
        <ul>
          <li>新增悬浮球图标替换，目前有日月西/象牙塔/极光海三种款式</li>
          <li>修复切换预设时仍注入日月西的思维链问题</li>
          <li>修复切换Claude时【以…回复】填入&lt;electric&gt;</li>
          <li>优化悬浮球消失与不好拖拽问题</li>
          <li>优化部分页面手机端的显示问题</li>
          <li>快捷开关各模块加入了模块开关的相关指引，查看更方便</li>
          <li>首页与预设管理新增【NSFW智能判断与世界书联动】（by &lt;@1338858632552648715&gt; &amp; &lt;@742637457488478229&gt; @果实）
            <ul>
              <li>根据摘要的NSFW回合数与上下文的关键词自动判断是否需要打开预设NSFW，可以在面板自行修改激活词</li>
              <li>可联动全局/角色卡世界书，检测到NSFW相关条目时，控制蓝灯、绿灯不发送，避免重复的NSGW内容发送给AI占用token</li>
              <li>NSFW自动判断激活时，悬浮球、qr栏的按钮会有相应激活态</li>
            </ul>
          </li>
        </ul>

        <div class="changelog-subtitle" role="heading" aria-level="3">日月来信</div>
        <ul><li>新增回信机制，可以写回信啦！</li></ul>
      </section>
      ...
      <section class="changelog-section">
        ...
        <ul>
          <li>把必开正则调整为了6个</li>
          <li>修复「日月西选开-隐藏多余内容」过于宽松导致的各种卡开场白显示问题</li>
          <li>修复「日月西美化-浮生绘卷」显示问题</li>
        </ul>
      </section>

      <section class="changelog-section">
        <h2>📱电波手机使用指南</h2>
        <p>电波手机是运行在酒馆助手中的角色互动小手机。它会根据当前角色卡、酒馆正文和你的设置，生成私聊、空间动态、备忘、日历、钱包等手机内容。</p>

        <div class="changelog-subtitle" role="heading" aria-level="3">首次配置</div>
        <h3>API 连接</h3>
        <p>用于手机独立生成、空间刷新、翻译等功能。请填写 API 地址、密钥和模型，保存前可使用连接测试确认配置是否可用。</p>
        <h3>聊天与行为</h3>
        <p>可设置消息的生成方式、每轮回复数量、手机私聊与酒馆正文的关系，以及是否向酒馆正文提供手机聊天摘要。</p>
        <ul>
          <li><b>动态判断</b>：根据当前话题自动判断是否承接正文。</li>
          <li><b>正文联动</b>：私聊会参考正文场景与已确认事件。</li>
          <li><b>独立聊天</b>：手机聊天作为独立话题，不主动续写正文情节。</li>
        </ul>
        <h3>角色与联系人</h3>
        <p>单角色卡会自动识别当前 Char。多角色或群聊场景可在消息页管理联系人，并为 NPC 设置姓名、备注、人设、关系和头像。</p>

        <div class="changelog-subtitle" role="heading" aria-level="3">功能模块</div>
        <h3>消息</h3>
        <p>用于角色私聊和群聊。支持文本、表情、图片/媒体描述、语音、位置、链接、转账、红包、引用、撤回、转发和消息反应。</p>
        <ul>
          <li>回车只发送消息，点击纸飞机才会激活角色回复。</li>
          <li>可连续发送多条消息，再让角色统一回复。</li>
          <li>点击联系人头像或右上角功能键，可管理当前聊天对象与聊天偏好。</li>
        </ul>
        <h3>状态</h3>
        <p>展示角色的好感、社会关系、心情、隐藏想法和身体状态等内容。状态数据可以跟随酒馆正文更新，也可通过手机的独立生成更新。</p>
        <h3>备忘</h3>
        <p>包含普通便签与涂鸦笔记。内容可由角色随剧情生成，也可手动更新。展开卡片可查看详细内容或翻译，右下角可删除单条记录。</p>
        <h3>空间</h3>
        <p>包含角色动态、评论、点赞、NPC 主页与匿名树洞。空间会结合当前角色、NPC 设定和近期剧情生成社交内容。</p>
        <ul>
          <li>手动点击「更新」可单独生成空间动态。</li>
          <li>用户匿名发言、NPC、动态作者和评论区共用随机头像库。</li>
          <li>右上角可更换空间封面或进入对应设置。</li>
        </ul>
        <h3>钱包</h3>
        <p>展示余额、收支记录和生活账本。转账与红包的互动结果会记入对应的消息和账目状态。</p>
        <h3>日历</h3>
        <p>用于保存约定、日程与待办。可查看未完成事项，并结合设置中的城市展示当地天气。</p>
        <h3>浏览</h3>
        <p>保存角色的搜索、网页浏览与阅读痕迹。可在应用设置中选择搜索引擎和浏览服务地址。</p>
        <h3>音乐</h3>
        <p>支持搜索歌曲、播放、收藏、歌单、播放队列和一起听。需要在音乐设置中选择音乐源并配置对应 API。</p>

        <div class="changelog-subtitle" role="heading" aria-level="3">生成与数据同步</div>
        <p>电波手机有两种主要更新途径：</p>
        <ul>
          <li><b>跟随酒馆生成</b>：角色回复正文时，按设置的概率与模块限制一起生成手机变化。</li>
          <li><b>手动独立生成</b>：在对应应用中点击更新，只生成指定模块，不额外生成酒馆正文。</li>
        </ul>
        <p>在「设置 → 基础设置」中可调整正文读取深度、排除标签、排除角色卡和缓存。「世界书管理」用于指定哪些世界书及条目可供手机生成使用。</p>

        <div class="changelog-subtitle" role="heading" aria-level="3">设置模块</div>
        <ul>
          <li><b>API 连接</b>：生成模型、接口、参数与连接测试。</li>
          <li><b>基础设置</b>：上下文、缓存、排除标签与运行范围。</li>
          <li><b>聊天与行为</b>：回复方式、联动策略、回复数量和翻译。</li>
          <li><b>世界书管理</b>：世界书来源与条目读取规则。</li>
          <li><b>语音与媒体</b>：头像压缩、图片质量与语音服务。</li>
          <li><b>通知与音效</b>：界面提示、回复提醒与自定义提示音。</li>
          <li><b>备份与恢复</b>：导出或导入 ZIP 备份。</li>
          <li><b>调试工具</b>：查看同步状态、提示词与运行日志。</li>
          <li><b>版权与致谢</b>：查看版权声明、使用边界与鸣谢清单。</li>
        </ul>
        <p>桌面图标、壁纸、状态栏、字体和字号等外观选项，可通过主屏外观入口调整。</p>

        <div class="changelog-subtitle" role="heading" aria-level="3">备份、隐私与迁移</div>
        <ul>
          <li>手机设置、角色资料和当前聊天数据保存在酒馆变量中。</li>
          <li>ZIP 备份可能包含 API 密钥，请勿将个人备份公开上传或转发。</li>
          <li>恢复备份前建议先导出一份当前数据，方便回退。</li>
          <li>切换角色卡或聊天文件时，手机会切换到对应的独立数据。</li>
        </ul>
      </section>
    `,
  },
